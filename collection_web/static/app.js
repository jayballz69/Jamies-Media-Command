...
async function fetchState({render, quiet}) {
  try {
    ...
    const fingerprint = JSON.stringify([next.collections, next.library, next.new_arrivals, next.drift, next.activity, next.settings, next.last_rotation_at, list(next.jobs).map(job => [job.id, job.status])]);
    let completed = false;
