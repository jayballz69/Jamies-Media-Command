...
function permanentShelves() { return published().filter(c => c.origin !== 'drift' || c.permanent); }
...
function renderCollections() {
  return `${heading('Collections', 'A place for the stories that belong together.', button('Import', 'import-dialog', {kind: 'secondary', glyph: 'download'}) + button('New collection', 'create', {glyph: 'plus'}))}<div class="browse-toolbar"><div class="filter-group" aria-label="Filter collections">${[['all','All collections'],['published','Published'],['draft','Drafts'],['movies','Movies'],['shows','TV shows'],['archived','Archived'],['drift-history','Drift history']].map(([value, label]) => `<button class="filter-chip" data-filter="${value}" aria-pressed="${filter === value}">${label}</button>`).join('')}</div><label class="search-field">${icon('search')}<span class="sr-only">Search collections</span><input id="collection-search" type="search" placeholder="Find a collection…" value="${esc(query)}" autocomplete="off"></label></div><div id="collection-results">${collectionResults()}</div>`;
}
...
async function fetchState({render, quiet}) {
  try {
    ...
    const fingerprint = JSON.stringify([next.collections, next.library, next.drift, next.activity, next.settings, next.last_rotation_at, list(next.jobs).map(job => [job.id, job.status])]);
    let completed = false;
...
document.addEventListener('submit', async event => {
  ...
  if (!['login-form', 'settings-form', 'rotation-form', 'create-form', 'collection-edit-form', 'trakt-form', 'discovery-form'].includes(form.id)) return;
  event.preventDefault();
  const submit = form.querySelector('[type=submit]');
  const errorId = {'login-form': 'login-error', 'settings-form': 'settings-error', 'rotation-form': 'rotation-error', 'create-form': 'create-error', 'collection-edit-form': 'edit-error', 'trakt-form': 'trakt-error', 'discovery-form': 'discovery-error'}[form.id];
