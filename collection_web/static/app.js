...
try { if (localStorage.getItem('collection-view') === 'list') collectionView = 'list'; } catch {}
let scheduleOpen = false;
let arrivalInboxOpen = false;
let query = '';
let detailId = null;
...
function renderOverview() {
  ...
  if (hasLibrary) return renderLibraryDashboard();
...
function scheduleSummary() {
  const s = state.settings || {}, a = s.advanced || {};
  const schedules = [
    ['Permanent shelves', nextCycle(state.last_rotation_at, s.rotation_hours ?? 24, a.schedule_enabled && a.home_enabled)],
    ['Displayed Drift shelves', state.drift?.diagnostics?.needs_attention ? 'Paused: check Drift' : nextCycle(state.drift?.last_activated_at, s.drift_interval_hours ?? 48, a.drift_schedule_enabled && a.auto_publish && a.home_enabled)],
    ['New Drift ideas', state.drift?.diagnostics?.needs_attention ? 'Paused: check Drift' : nextCycle(state.drift?.last_generated_at, s.drift_generation_hours ?? 168, a.drift_schedule_enabled)],
  ];
  return `<div class="schedule-summary">${schedules.map(([label, timing]) => `<div><span>${esc(label)}</span><strong>${esc(timing)}</strong></div>`).join('')}</div>`;
}
function renderLibraryDashboard() {
  const home = published().filter(c => c.home);
  const attention = [];
  if (state.library_diagnostics?.needs_attention) attention.push(['Library sync needs attention', 'activity', 'View Activity']);
  if (state.drift?.diagnostics?.needs_attention) attention.push(['Drift generation is paused', 'drift', 'Check Drift']);
  const latest = new Map();
  for (const job of list(state.jobs)) if (!latest.has(job.kind)) latest.set(job.kind, job);
  const failures = [...latest.values()].filter(j => j.status === 'failed');
  if (failures.length) attention.push([`${num(failures.length)} tasks need checking`, 'activity', 'View Activity']);
  const paused = activeCollections().filter(c => [...list(c.available), ...list(c.missing)].some(i => i.arrival_review_error || i.request_error));
  if (paused.length) attention.push([`${num(paused.length)} collections have paused additions or requests`, 'collections', 'Review collections']);
  if (state.family_shelf?.error) attention.push(['Family shelf needs attention', 'rotation', 'Check family shelf']);
  if (arrivalSuggestions().length) attention.push([`${num(arrivalSuggestions().length)} new arrivals have collection suggestions`, 'review-arrival-inbox', 'Review suggestions']);
  const drafts = activeCollections().filter(c => ['draft', 'kept'].includes(c.status) &&
    !(c.origin === 'drift' && !c.permanent && state.settings?.advanced?.auto_publish && list(state.drift?.pool_ids).includes(c.id)));
  if (drafts.length) attention.push([`${num(drafts.length)} drafts ready to review`, 'review-drafts', 'Review drafts']);
  return `${heading('Your library at a glance.', `${num(state.library?.count)} titles · ${num(activeCollections().length)} collections`, button('New collection', 'create', {kind:'secondary',glyph:'plus'}))}
    <section class="dashboard-attention" aria-labelledby="attention-title"><h2 id="attention-title">Needs your attention</h2>${attention.length ? attention.map(([label, action, cta]) => `<div class="attention-row"><p>${esc(label)}</p>${button(cta,action,{kind:'ghost',small:true})}</div>`).join('') : '<p class="all-clear">All clear. No tasks need your attention.</p>'}</section>
    <section class="section" data-overview-home><div class="section-heading"><h2>On Plex Home now</h2>${button('Manage shelves','rotation',{kind:'ghost',small:true})}</div>${home.length ? `<div class="home-shelf-grid">${home.map(c => `<button class="home-shelf" data-action="open" data-id="${esc(c.id)}">${icon(typeIcon(c.media_type))}<span><strong>${esc(c.name)}</strong><small>${esc(typeName(c.media_type))} · ${c.pinned_home ? 'Pinned' : c.origin === 'drift' && !c.permanent ? 'Drift' : 'Permanent'}</small></span></button>`).join('')}</div>` : '<p>No collections are displayed yet. Publish a collection, then choose its Home rotation.</p>'}</section>
    <section class="section" data-next-changes><div class="section-heading"><h2>Next scheduled changes</h2>${button('Preview next shelves','rotation-preview',{kind:'ghost',small:true})}</div>${scheduleSummary()}</section>`;
}
function filteredCollections() {
  return list(state.collections).filter(c => ['archived', 'drift-history'].includes(filter) ? c.status === 'archived' : c.status !== 'archived').filter(c => {
    ...
    if (mediaFilter === 'movies' && ['show', 'tv', 'series'].includes(c.media_type)) return false;
    if (mediaFilter === 'shows' && !['show', 'tv', 'series'].includes(c.media_type)) return false;
    return `${c.name} ${c.description || ''} ${c.thesis || ''}`.toLowerCase().includes(query.toLowerCase());
  }).sort((a,b) => {
    if (collectionSort === 'recent') return Number(b.created_at || 0) - Number(a.created_at || 0) || String(a.name).localeCompare(String(b.name));
    if (collectionSort === 'missing') return list(b.missing).length - list(a.missing).length || String(a.name).localeCompare(String(b.name));
    if (collectionSort === 'home') return Number(!!b.home) - Number(!!a.home) || String(a.name).localeCompare(String(b.name));
    return String(a.name).localeCompare(String(b.name));
  });
}
function collectionResults() {
  const found = filteredCollections();
  const filtered = query || filter !== 'all' || mediaFilter !== 'all';
  return `<p class="browse-summary" aria-live="polite">${num(found.length)} ${found.length === 1 ? 'collection' : 'collections'}${query ? ` matching “${esc(query)}”` : ''}</p>${found.length ? collectionView === 'list' ? collectionTable(found) : `<div class="collection-grid">${found.map((c, i) => collectionCard(c, i)).join('')}</div>` : empty(filtered ? 'No collections match just yet' : 'Build something worth browsing', filtered ? 'Try another search or choose a different filter.' : 'Gather titles around an idea, import a list, or explore what Drift finds in your library.', filtered ? '' : 'create', 'Create a collection')}`;
}
function collectionTable(rows) {
  return `<table class="collection-table"><caption class="sr-only">Collections and their current placement</caption><thead><tr>${['Collection','Type','State','Home','Missing','Rotation'].map(label=>`<th scope="col">${label}</th>`).join('')}</tr></thead><tbody>${rows.map(c=>`<tr><td class="collection-name" data-label="Collection"><button class="text-link" data-action="open" data-id="${esc(c.id)}">${esc(c.name)}</button><small>${num(list(c.items).length)} titles</small></td><td data-label="Type">${esc(typeName(c.media_type))}</td><td data-label="State">${pill(c)}</td><td data-label="Home">${c.home && c.status === 'published' ? c.pinned_home ? 'Pinned on Home' : 'On Home' : 'Off Home'}</td><td data-label="Missing">${num(list(c.missing).length)}</td><td data-label="Rotation">${c.status === 'archived' ? 'Archived' : c.pinned_home ? 'Pinned' : c.managed === false ? 'Managed elsewhere' : c.status !== 'published' ? 'Not published' : c.rotation_enabled ? c.origin === 'drift' && !c.permanent ? 'Drift pool' : 'In pool' : 'Off'}</td></tr>`).join('')}</tbody></table>`;
}
...
function renderCollections() {
  return `${heading('Collections', 'A place for the stories that belong together.', button('Review permanent collections', 'collection-sweep', {kind:'secondary',glyph:'drift',disabled:running()}) + button('Import', 'import-dialog', {kind: 'secondary', glyph: 'download'}) + button('New collection', 'create', {glyph: 'plus'}))}<div class="browse-toolbar"><div class="browse-controls"><label class="field"><span>Media</span><select id="collection-media">${[['all','All media'],['movies','Movies'],['shows','TV shows']].map(([value,label])=>`<option value="${value}" ${mediaFilter === value ? 'selected' : ''}>${label}</option>`).join('')}</select></label><label class="field"><span>State</span><select id="collection-status">${[['all','All active'],['published','Published'],['draft','Drafts'],['archived','Archived'],['drift-history','Drift history']].map(([value,label])=>`<option value="${value}" ${filter === value ? 'selected' : ''}>${label}</option>`).join('')}</select></label><label class="field"><span>Sort by</span><select id="collection-sort">${[['name','Name'],['recent','Newest created'],['missing','Most missing'],['home','On Home first']].map(([value,label])=>`<option value="${value}" ${collectionSort === value ? 'selected' : ''}>${label}</option>`).join('')}</select></label><div class="view-switch" role="group" aria-label="Collection view">${[['grid','Grid'],['list','List']].map(([value,label])=>`<button class="filter-chip" data-view="${value}" aria-pressed="${collectionView === value}">${label}</button>`).join('')}</div></div><label class="search-field">${icon('search')}<span class="sr-only">Search collections</span><input id="collection-search" type="search" placeholder="Find a collection…" value="${esc(query)}" autocomplete="off"></label></div>${state.collection_review ? `<details class="settings-details"><summary>${icon('drift')}<span><strong>Collection review</strong><small>${esc(date(state.collection_review.created_at))} - suggestions only</small></span>${icon('chevron')}</summary><div class="advanced-body"><p class="detail-note">${esc(state.collection_review.summary)}</p>${list(state.collection_review.collections).map(row => `<details class="section"><summary>${esc(row.name)}</summary><p class="detail-note">${esc(row.flavour)}</p><p class="detail-note">${esc(row.advice)}</p>${list(row.additions).map(item=>`<div class="media-item"><div class="item-info"><strong>${esc(item.title)} (${esc(item.year)})</strong><p>${esc(item.reason)}</p></div></div>`).join('')}${button('Open collection','open',{id:row.id,kind:'ghost',small:true})}</details>`).join('')}</div></details>` : ''}<details class="settings-details" id="arrival-inbox-shell" ${arrivalInboxOpen ? 'open' : ''}><summary><span><strong>New arrival suggestions (${num(arrivalSuggestions().length)})</strong></span>${icon('chevron')}</summary><div class="advanced-body">${renderArrivalInbox()}</div></details><div id="collection-results">${collectionResults()}</div>`;
}
function renderDrift() {
  ...
  return `${heading('A fresh perspective.', `${num(slots.drift - slots.driftTV)} movie shelves and ${num(slots.driftTV)} TV shelves alongside your permanent favourites.`, button('Drift history', 'drift-history', {kind: 'secondary', glyph: 'clock'}) + button('Switch displayed shelves', 'switch-drift', {kind: 'secondary', glyph: 'rotation', disabled: running('drift') || !list(state.drift?.pool_ids).length}) + button('Generate ideas', 'generate', {glyph: 'drift', disabled: running('drift') || !state.library?.count}), 'DRIFT')}
    ...
    ${diagnostics.needs_attention ? '<p class="detail-note" role="status">Scheduled exploration is paused after repeated unsuccessful batches. Check the details below, then Generate ideas manually to resume.</p>' : ''}
...
function renderFamilyShelf() {
  ...
  return `<section class="quiet-panel section" data-family-shelf><div class="section-heading"><div><span class="eyebrow">PINNED FAMILY MOVIE NIGHT</span><h2>${esc(c?.name || 'Family shelf')}</h2><p>${num(list(c?.items).length)} / 25 movies &middot; ${config.enabled ? 'Updates automatically after Plex sync' : 'Automatic updates paused'}</p></div></div><p>The newest arrivals for ages 10?16 and the grown-ups. Audience scores must be at least 6/10 when IMDb or TMDb has 100+ votes; unrated and low-vote films can still qualify. Older films move into ${esc(getCollection(config.destination_id)?.name || 'your family collection')}. This shelf stays on Home alongside your rotating shelves.</p><div class="actions">${button('Open shelf','open',{id:config.shelf_id,kind:'secondary',small:true})}${button('Refresh family shelf','family-refresh',{kind:'secondary',small:true,disabled:running()})}${button(config.enabled ? 'Pause automatic updates' : 'Resume automatic updates','family-toggle',{kind:'ghost',small:true,disabled:running()})}</div>${config.audience_error ? `<p class="detail-note">${esc(config.audience_error)}</p>` : ''}${config.error ? `<p class="form-error">${esc(config.error)}</p>` : ''}</section>`;
}
function rotationSlots() {
  const slots = shelfSlots();
  const permanent = permanentShelves().filter(c => c.home);
  const drift = activeDrift().filter(c => c.home);
  const groups = [['Permanent movies',permanent.filter(c=>typeIcon(c.media_type)==='film').length,slots.movies],['Permanent TV',permanent.filter(c=>typeIcon(c.media_type)==='tv').length,slots.shows],['Drift movies',drift.filter(c=>typeIcon(c.media_type)==='film').length,slots.drift-slots.driftTV],['Drift TV',drift.filter(c=>typeIcon(c.media_type)==='tv').length,slots.driftTV]];
  return `${!state.settings?.advanced?.home_enabled ? '<p class="notice">Plex Home updates are disabled in Advanced settings.</p>' : ''}<div class="rotation-slots" aria-label="Occupied Home slots">${groups.map(([label,count,total])=>`<article class="slot-card"><span>${label}</span><strong>${num(count)} <small>/ ${num(total)}</small></strong><div class="slot-track" aria-hidden="true">${Array.from({length:total},(_,i)=>`<i class="${i<count?'filled':''}"></i>`).join('')}</div><p>${total ? count>total ? 'Extra shelves remain until rotation' : `${Math.max(0,total-count)} open slots` : 'No slots configured'}</p></article>`).join('')}</div>`;
}
function renderRotation() {
  ...
  return `${heading('Keep Home moving.', 'Permanent favourites and temporary discoveries, side by side.', button('Preview next shelves', 'rotation-preview', {kind: 'secondary', glyph: 'clock'}) + button('Rotate permanent shelves', 'rotate', {glyph: 'rotation', disabled: (!inPool.length && !onHome.some(c => c.managed)) || running('rotation')}), 'ROTATION')}
    ${rotationSlots()}
    ${scheduleSummary()}
    <details class="settings-details section" id="rotation-schedule" ${scheduleOpen || rotationEdits ? 'open' : ''}><summary><span><strong>Edit schedule</strong><small>Shelf counts and timing</small></span>${icon('chevron')}</summary><div class="advanced-body">${rotationSetup()}</div></details>
    <section class="section" data-permanent-home><div class="section-heading"><div><h2>Permanent shelves</h2><p>${num(onHome.filter(c => typeIcon(c.media_type) === 'film').length)} / ${num(slots.movies)} movies · ${num(onHome.filter(c => typeIcon(c.media_type) === 'tv').length)} / ${num(slots.shows)} TV shows · ${esc(nextCycle(state.last_rotation_at, settings.rotation_hours ?? 24, settings.advanced?.schedule_enabled && settings.advanced?.home_enabled))}</p></div></div>${onHome.length ? `<div class="collection-grid">${onHome.map((c, i) => collectionCard(c, i)).join('')}</div>` : empty('Your favourites have room here', 'Add published collections to the permanent pool, then rotate them onto Home.', '', '', 'collections')}</section>
    <section class="section" data-drift-live><div class="section-heading"><div><h2>Drift shelves</h2><p>${num(liveDrift.filter(c => c.home).length)} / ${num(slots.drift)} on Home · ${esc(nextCycle(state.drift?.last_activated_at, settings.drift_interval_hours ?? 48, settings.advanced?.drift_schedule_enabled && settings.advanced?.auto_publish && settings.advanced?.home_enabled))}</p></div>${button('Generate ideas', 'generate', {kind: 'secondary', glyph: 'drift', disabled: running('drift') || !state.library?.count})}</div>${liveDrift.length ? `<div class="collection-grid">${liveDrift.map(driftCard).join('')}</div>` : empty('A little space for discovery', 'Generate ideas to create a batch of temporary collections alongside your permanent shelves.', '', '', 'drift')}</section>
    ${renderFamilyShelf()}
...
async function openRotationPreview() {
  ...
  try {
    ...
    createDialog.innerHTML = `<h2 id="create-title">Up next on Home</h2><p>Based on your saved settings and current pool. Permanent rotation and Drift switch separately.</p>
      ...
      ${group('Drift shelves', preview.drift, preview.pool_refresh_due ? 'For Switch displayed shelves using the current pool. A new weekly or seasonal generation is due and may change the scheduled choices.' : preview.drift_scheduled ? 'For the next switch within this weekly pool. Seasonal shelves have priority while their occasion is active.' : 'For Switch displayed shelves using the saved pool.')}
...
function renderSettings() {
  ...
  return `${heading('Make it yours.', 'Your connections and preferences, all in one place.', '', 'SETTINGS')}
    <div class="settings-layout"><form id="settings-form">
      ...
      ${toggle('drift_schedule_enabled', 'Schedule Drift generation and shelf switches', 'Generate a weekly pool and switch through its saved collections on your Drift schedule.', !!a.drift_schedule_enabled)}
...
document.addEventListener('click', async event => {
  ...
  if (view) {
    collectionView = view.dataset.view === 'list' ? 'list' : 'grid';
    try { localStorage.setItem('collection-view', collectionView); } catch {}
    renderPage(); document.querySelector(`[data-view="${collectionView}"]`)?.focus(); return;
  }
  ...
  if (action === 'review-arrival-inbox') { arrivalInboxOpen = true; if (page === 'collections') renderPage(); else navigate('collections'); return; }
  if (action === 'review-drafts') { filter = 'draft'; mediaFilter = 'all'; query = ''; if (page === 'collections') renderPage(); else navigate('collections'); return; }
  if (action === 'drift-history') { filter = 'drift-history'; mediaFilter = 'all'; query = ''; if (page === 'collections') renderPage(); else navigate('collections'); return; }
...
document.addEventListener('toggle', event => {
  if (event.target.id === 'rotation-schedule' && event.target.isConnected) scheduleOpen = event.target.open;
...
document.addEventListener('change', async event => {
  if (['collection-media', 'collection-sort', 'collection-status'].includes(event.target.id)) {
    ...
    else collectionSort = event.target.value;
    document.querySelector('#collection-results').innerHTML = collectionResults();
    return;
  }
